...
[![PyPI](https://img.shields.io/pypi/v/cognis-envdoctor.svg)](https://pypi.org/project/cognis-envdoctor/)
[![CI](https://github.com/cognis-digital/envdoctor/actions/workflows/ci.yml/badge.svg)](https://github.com/cognis-digital/envdoctor/actions)
...
ENVDOCTOR exists for one job — .env validator, secret-presence and config-drift checker — and does it without a SaaS bill or heavyweight setup.
...
```bash
envdoctor --version
envdoctor scan .                      # scan the current project
envdoctor scan . --format json
envdoctor scan . --fail-on high       # non-zero exit for CI gates
envdoctor mcp                         # expose as an MCP server (Cognis.Studio / Claude Desktop / Cursor)
```
...
Built in the spirit of **dotenv-linter**, re-framed for the Cognis approach: single-purpose, self-hostable,
MCP-native, and unified with the rest of the Suite. Missing a credit? Open a PR.

## How it fits the Cognis Neural Suite

`envdoctor` is one of the **100+ tools** in the [Cognis Neural Suite](https://github.com/cognis-digital).
